#!/bin/bash
# Usage: ./adhocSyncStats.sh folder1 folder2 output_folder

#set -x

if [ ! $# -eq 3 ]; then
  echo -e "Usage: ./adhocSyncStats.sh folder1 folder2 output_folder"
  exit 1
fi
...
do
    IFDIFFERENT=1
    prev_sum=$(grep $f1 md5_folder1 | awk '{print $1}')
    for f2 in $FOLDER2/*
    do
        sum=$(grep $f2 md5_folder2 | awk '{print $1}')
