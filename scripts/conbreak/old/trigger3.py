...
def set_trigger():
	...
	bp_read = target.BreakpointCreateByLocation("mod_mem_cache.c", 783)
	bp_write = target.BreakpointCreateByLocation("mod_mem_cache.c", 674)

	bp_read.SetScriptCallbackFunction("trigger3.read_callback")
	bp_write.SetScriptCallbackFunction("trigger3.write_callback")
...
def write_callback(frame, bp_loc, dict):
	...
	COUNT += 1
	if len(OBJ_ARR) < 5:
		#print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>" + str(frame.FindVariable("nelts"))
		
		obj = str(frame.FindVariable("nelts")).split()[-1]
		OBJ_ARR.append(obj)
...
def read_callback(frame, bp_loc, dict):
	...
	#temp = str(frame.FindVariable("mobj").GetChildMemberWithName("num_req_hdrs").GetAddress())
	#print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>" + temp

	global OBJ_ARR
	print "INFO:  OBJ_ARR = " + str(OBJ_ARR)

	key1 = str(frame.FindVariable("mobj")).split()[-1]
	key2 = str(frame.FindVariable("mobj").GetChildMemberWithName("num_req_hdrs").GetAddress())
